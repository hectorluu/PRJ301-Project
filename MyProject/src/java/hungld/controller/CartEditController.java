...
public class CartEditController extends HttpServlet {
    ...
    private static final String DELETE = "CartRemoveController";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (quantity < 1) {
                ...
                for (FlowerDTO dto : cart.getCart().values()) {
                    if (dto.getProductID().equals(productID)) {
                        // set flowerdto
                        String productName = dto.getProductName();
                        float price = (float) dto.getPrice();
                        String categoryID = dto.getCategoryID();
                        flower = new FlowerDTO(productID, productName, quantity, price, categoryID);
                        // dung` break de toi uu vi 1 lan duy nhat 1 loai.
                        break;
                    }
                }
                cart.update(productID, flower);
                session.setAttribute("CART", cart);
                url = SUCCESS;
            }

        } catch (Exception ex) {
            log("Exception at _ CartEditController :" + ex.getMessage());
        } finally {
